...
{
    qDebug() << "#############################";
    qDebug() << "Ovo je iz updateGeometry(); ";
    qDebug() << "Anchor je : " << m_anchor;
    //    qDebug() << "Dok je problem sa tackom: " << m_chart->mapToPosition(m_anchor);
    //    qDebug() << "Hmmm dok bi trebalo da bude " << m_chart->mapToParent(m_anchor);
    qDebug() << "Ovo je mapToPosition" << m_chart->mapToPosition(m_anchor);
    qDebug() << "Ovo je mapToParent()" << mapToParent(m_chart->mapToPosition(m_anchor));

    qDebug() << "#############################";
    //    qDebug() << "Parent od ovog govneta od itema je " << this->parentObject();
    prepareGeometryChange();
    //Ovaj smo morali da promenimo zbog QPointa... posto je velika promena za
    // nase vrednosti
    setPos(m_chart->mapToPosition(m_anchor) + QPoint(10, -50));

    //Aj probamo ovako ,ne radi ni ovako
